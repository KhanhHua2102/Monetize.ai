...
def generate_profit(input):
    global context_data
    response_values = input.split()
    if len(input) == 4 and response_values[0].isdigit() and all(isinstance(val, str) for val in response_values[1:]):
        ...
        if error_msg:
            ...
            prompt = context_data + 'Q: Using this information to give users a message:' + response + '\n'
...
def generate():
    ...
    print(userMessage + '\n')
    if contains_stock_info(userMessage):
    # perform a specific action for when stock information is present
        ...
        context_data += 'Q: Convert this data so that it is able to input into Yfinance, must not caculate profit: {number of Shares} {Ticker} {Start-Date} {End-Date} must not caculate profit, User message: '+ userMessage +'Response must follow formats of convertsion only no need any comments: No need any Punctuation, the dates must be converted to dd/mm/yyyy, default end date is the word today no need to assume.\nA: '
        result = generate_profit(userMessage)
        context_data += result
